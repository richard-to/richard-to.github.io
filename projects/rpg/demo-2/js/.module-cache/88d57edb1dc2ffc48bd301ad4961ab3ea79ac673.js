...
(function(window, undefined) {
    ...
    SpriteHighlightEffect.prototype.draw = function(x, y, tileSize, scale) {
        var radius = this.radius * scale;
        ctx.beginPath();
        ctx.globalAlpha = 0.8;
        ctx.arc(
            (x - sx) * tileSize + tileSize / 2,
            (y - sy) * tileSize + tileSize - this.radius * this.scale,
            radius, 0, 2 * Math.PI, false);
        ctx.fillStyle = '#4fb3c6';
        ctx.fill();
        ctx.lineWidth = 3;
        ctx.strokeStyle = '#a2e4ef';
        ctx.stroke();
        ctx.globalAlpha = 1.0;
    };
    ...
    SpriteRenderer.prototype.draw = function(ctx, sprite) {
        ...
        var radius = (32 * scale) - (20 * scale);
        ctx.beginPath();
        ctx.globalAlpha = 0.8;
        ctx.arc(
            (x - sx) * tileSize + tileSize / 2,
            (y - sy) * tileSize + tileSize - radius,
            radius, 0, 2 * Math.PI, false);
        ctx.fillStyle = '#4fb3c6';
        ctx.fill();
        ctx.lineWidth = 3;
