...
(function(window, undefined) {
    ...
    var LevelLoader = function(el, ctx, datastore, options) {
        this.options = $.extend({
            assets: {
                map: 'map.json',
            ...
            dataLoaders: rpg.util.dataLoaders,
            tiles: {
                grass: 'grass.png',
                water: 'water2.png',
                cliff: 'cliff.png'
    ...
    LevelLoader.load = function(onLoad) {
        var self = this;
        this.datastore.load(this.options.assets, function() {
            self.onAssetsLoad();
            onLoad();
        }, this.options.dataLoaders);
    };

    // Once external assets are loaded, everything else is initialized.
    LevelLoader.prototype.onAssetsLoad = function() {
        this.initTiles();
        this.initSprites();
        this.initHero();
    };
    ...
    //
    LevelLoader.prototype.initTiles = function() {
        this.tileLookup = [];
        this.tileFactory = {};
    ...
    LevelLoader.prototype.initSprites = function() {
    ...
    LevelLoader.prototype.initHero = function() {
    ...
    //
    var GameEngine = function(el, datastore, options) {
        this.options = $.extend({
            tileSize: 64,
            gridWidth: 10,
            gridHeight: 7
    ...
    GameEngine.prototype.loadLevel = function(levelLoader) {
        var self = this;
        this.levelLoader = levelLoader;
        this.levelLoader.load(function() {
            self.animLoop = new AnimLoop(ctx, heroSprite, sprites, bgRenderer, spriteRenderer)
        });
        var self = this;
