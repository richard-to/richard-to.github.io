(function(window, undefined) {
    ...
    HeroSprite.prototype.attack = function() {
        this.moveLeft();
        this.frameQueue.unshift([this.frames.attack_1, this.x, this.y, 10]);
        this.frameQueue.unshift([this.frames.attack_2, this.x-1, this.y, 15]);
        this.frameQueue.unshift([this.frames.face_left, this.x, this.y, 0]);
        this.backTrack();

    };

    // Draws character backtracking to the right.
    HeroSprite.prototype.backTrackRight = function() {
        var frames = this.frames;
        var y = this.y;
        var x = this.x + 1;
        if (x < this.map.length && this.map[y][x] == 0) {
