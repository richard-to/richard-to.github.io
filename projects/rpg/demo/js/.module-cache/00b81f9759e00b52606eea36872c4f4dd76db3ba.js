...
(function(window, undefined) {
    ...
    var EnemyMenu = React.createClass({displayName: 'EnemyMenu',
        render: function() {
            var createItem = function(enemy) {
                return (
                    React.DOM.tr(null, 
                        React.DOM.td( {className:"name"}, enemy.attr.name),
                        React.DOM.td( {className:"hp"}, enemy.attr.hp,"/",enemy.attr.hpMax),
                        React.DOM.td( {className:"mp"}, enemy.attr.mp,"/",enemy.attr.mpMax)
                    )
                )
            };
            return (
                React.DOM.div( {class:"enemy-wrap"}, 
    ...
    var PartyMenu = React.createClass({displayName: 'PartyMenu',
        render: function() {
            ...
            return (
                React.DOM.div( {class:"party-wrap"}, 
                React.DOM.table( {className:"entity-status"}, 
                    React.DOM.tr(null, React.DOM.td(null),React.DOM.td(null, "HP"),React.DOM.td(null, "MP")),
        ...
        render: function() {
            ...
            return(
                ...
                React.DOM.table( {className:"hero-status"}, 
                    React.DOM.tr(null, React.DOM.td(null),React.DOM.td(null, "HP"),React.DOM.td(null, "MP")),
                    React.DOM.tr(null, 
                        React.DOM.td( {className:"name", onClick:this.handleTest}, this.props.enemy.attr.name),
            ...
            if (this.props.showAction) {
                ...
                return (
                    React.DOM.div( {onClick:this.handleClick}, 
                        EnemyMenu( {enemy:this.props.enemy, showEnemy:this.state.showEnemy, onSearching:this.handleTest} )
                    )
                );
            }
        }
    ...
    var HeroStatsApp = React.createClass({displayName: 'HeroStatsApp',
        ...
        render: function() {
            return (
                React.DOM.div(null, 
                    ActionMenu( {showAction:this.state.showAction, enemy:this.props.enemy, items:this.props.players} ),
